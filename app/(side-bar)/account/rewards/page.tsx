...
function AccountRewards() {
  ...
  return (
    <div className="mt-10 max-w-2xl mx-auto">
        ...
        <p className="p-4 text-md text-gray-700 font-semibold">You have {points} earned. </p>
        <div className="relative w-full">
            <Progress className="h-7 [&>div]:bg-red-500 transition" value={Math.min(200, points / 2)} />
            <span className={"absolute inset-0 flex items-center justify-center text-sm font-semibold text-gray-100"}>
                {Math.min(200, points / 2)}%
            </span>
        </div>
    </div>
  )
